...
def csv_reader_14(
    ...
    if 'LinRegr_Pix.csv' == Func_Parm_CSV :
        for line in reader:
            N_line     += 1       	
            dct[Func_Parm_CSV][line['coef']] = line['intercept'] # coef	intercept

    elif 'pix_price.csv' == Func_Parm_CSV :
        for line in reader:
            N_line     += 1  
            dct[Func_Parm_CSV][line['Pixel_Pitch_avr']] = line 

    elif 'pix_price_add.csv' == Func_Parm_CSV :
        for line in reader:
            N_line     += 1  
            dct[Func_Parm_CSV][line['Pixel_Pitch_avr']] = line 
...
def Handle_diagonal_pixels(
    ...
    # Life time  >100,000 Hours (50%) 
    Life_Time_Hours  =  200000.0  # Hours      (~100%) 
    Life_Time_Days   =  Life_Time_Hours/24     # (8333)
    Life_Time_Months =  Life_Time_Hours/24/12     # (8333)
    Life_Time_Years  =  Life_Time_Hours/24/365 # (~23) 

    f_money_dol_day =  float(money_dol_day)
    ...
    if ppi in dct['pix_price_add.csv']:
        val = dct['pix_price_add.csv'][ppi]
        print('ppi =',ppi,'=>',val)
        #lst = val['dollar_avr']
        sum_1_m = float(val['dollar_avr'])
    else:
        sum_1_m = 1.0


    ##s_dol = float(area) * p * 218.94 + 2744.03
    ##s_dol = float(area) * (p * coef + intercept)
    s_dol = float(area) * sum_1_m

    scr_dol = str(round(s_dol, 2))

    diagonal_pixels = str(round(d, 2))
    s_Life_Time_Months = str(round(Life_Time_Months, 2))
    f_payback_Months  = s_dol/(f_money_dol_day*30)
    payback_Months    = str(round(f_payback_Months, 2))


    f_profit_Months = Life_Time_Months -f_payback_Months
    f_profit_Total = f_money_dol_day * 30 * f_profit_Months - s_dol

    profit_Months   = str(round(f_profit_Months, 2))
    profit_Total    = str(round(f_profit_Total, 2))
    ...
    print('ppi =',ppi)
    print('sum_1_m =',sum_1_m)
    print('scr_dol =',scr_dol)
    print('Life_Time_Months =',s_Life_Time_Months)
    print('payback_Months =',payback_Months)
    print('profit_Months =',profit_Months)
    print('profit_Total =',profit_Total)

    return diagonal_pixels,\
        ppi,scr_dol,s_Life_Time_Months,\
        payback_Months,profit_Months,profit_Total
#-------------------------------------------------------------------------------
...
def csv_reader_14_dtl(
    ...
    for coef in dct['LinRegr_Pix.csv']:
        intercept = dct['LinRegr_Pix.csv'][coef]
    ...
    if   'usd_rub.csv' == Func_Parm_CSV :
        ...
        for line in reader:
            ...
            payback_Months,\
            profit_Months,\
            profit_Total = Handle_diagonal_pixels(
                                                     dct,
                                                     line,
                                                     money_dol_day,
                                                     diag_in,
...
def akaMain(argv):
    ...
    dct['LinRegr_Pix.csv']= dict() 
    dct['pix_price.csv']  = dict() 
    dct['pix_price_add.csv']  = dict() 
    ...
    dct = Handle_csv_2_dict('LinRegr_Pix.csv' ,dct)
    dct = Handle_csv_2_dict('pix_price.csv' ,dct)
    dct = Handle_csv_2_dict('pix_price_add.csv' ,dct)
